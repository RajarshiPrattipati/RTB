...
func _ready():
	...
	# Call initialize_battle after a short delay to ensure all battlers are fully initialized
	call_deferred("_deferred_initialize")
	# Note: Button toggles handled by new SkillPanel UI
	# Old ActionButtons no longer exist in the new UI

func _deferred_initialize():
	# Wait additional frames to ensure all child _ready() functions have completed
	await get_tree().process_frame
	await get_tree().process_frame
	await get_tree().process_frame
	await get_tree().process_frame

	# Additional wait to ensure battlers have fully initialized
	await get_tree().create_timer(0.5).timeout

	# Debug: Check ALL children in the scene tree
	print("=== CHECKING ALL CHILDREN IN SCENE ===")
	for child in get_children():
		print("Child: ", child.name, " (Type: ", child.get_class(), ")")
		if child is Battler:
			print("  - Is Battler: true")
			print("  - Team: ", child.team)
			print("  - Groups: ", child.get_groups())

	# Check all nodes in the entire tree
	print("=== CHECKING ALL NODES IN TREE ===")
	var all_nodes = get_tree().root.get_children()
	for node in all_nodes:
		print("Root child: ", node.name)
		_print_node_tree(node, 1)
...
func _print_node_tree(node: Node, depth: int):
	...
	if node is Battler:
		print(indent, "  [BATTLER] team=", node.team, " groups=", node.get_groups())
	for child in node.get_children():
		_print_node_tree(child, depth + 1)

func _print_full_tree(node: Node, depth: int):
	var indent = ""
	for i in range(depth):
		indent += "  "
	var groups_str = ""
	if node.get_groups().size() > 0:
		groups_str = " [groups: " + str(node.get_groups()) + "]"
	print(indent, node.name, " (", node.get_class(), ")", groups_str)
	for child in node.get_children():
...
func initialize_battle():
	...
	# Debug: Check all nodes in groups
	print("=== DEBUG: Checking all groups ===")

	# Check what's in the entire scene tree
	print("\n=== ENTIRE SCENE TREE ===")
	var root = get_tree().root
	_print_full_tree(root, 0)

	var all_players = get_tree().get_nodes_in_group("players")
	var all_enemies = get_tree().get_nodes_in_group("enemies")
	var all_battler_data = get_tree().get_nodes_in_group("battler_data")

	print("\n=== GROUP CONTENTS ===")
	print("Nodes in 'players' group: ", all_players.size())
	for node in all_players:
		print("  - ", node.name, " (Type: ", node.get_class(), ", Is Battler: ", node is Battler, ")")
	print("Nodes in 'enemies' group: ", all_enemies.size())
	for node in all_enemies:
		print("  - ", node.name, " (Type: ", node.get_class(), ", Is Battler: ", node is Battler, ")")
		if node is Battler:
			print("    - character_name: ", node.character_name)
			print("    - team: ", node.team)
			print("    - stats: ", node.stats)

	for node in get_tree().get_nodes_in_group("players"):
		if node is Battler:
			players.append(node as Battler)
			print("Found player: ", node.character_name, " HP: ", node.current_health)

	for node in get_tree().get_nodes_in_group("enemies"):
		if node is Battler:
			enemies.append(node as Battler)
			print("Found enemy: ", node.character_name, " HP: ", node.current_health, " Defeated: ", node.is_defeated())

	print("=== BATTLE INITIALIZATION ===")
	print("Total players: ", players.size())
	print("Total enemies: ", enemies.size())

	# Safety check: don't start battle if no enemies or players
	if enemies.is_empty():
		push_error("Cannot start battle: No enemies found!")
		return
	if players.is_empty():
		push_error("Cannot start battle: No players found!")
		return
	...
	print("Turn order: ", turn_order.size(), " battlers")
...
func all_defeated(characters: Array):
	# Empty array should not count as "all defeated"
...
func update_button_states():
	# Old button system - no longer used with SkillPanel UI
	# Skills are now displayed directly in the panel
	pass
